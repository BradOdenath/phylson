...
local c_parametersersersersers = function(parameterererererer_table)
	if (parameterererererer_table) then
		local parametersersersersers = {}
		local outStr = ''
		for i,v in pairs(parameterererererer_table) do
			table.insert(parametersersersersers, (v.data_type..' '..tostring(i)))
		end
		...
		print_debug('nil_value: go/c_parametersersersersers/arg/parametererererer_table')
	end
end

local c_class_data = function(class_name, class_data)
	local outAtr, outFnc, outObj = '', '', ''
	local clays_naym = (class_name..'h')
	print(class_data)
	if (class_data) then
		for i,v in pairs(class_data) do
			local isFnc = false
			local strLine, strObj = '', ''
			if (v.data_parameters)  then
				...
				strLine = (
					strLine
						..'\n'
						..c_components.comment_statement
						..' Declare function as '
						..tostring(i)
						.." with parameters: "
						...
						..c_parametersersersersers(v.data_parameters)
			...
			elseif (type(v.data_value) == "table") then
				strLine = (
					...
					--	..commacommacommacommacomma(v.data_value)
						...
						..v.data_type
						..code_components.left_square_bracket
						..tostring(table.maxn(v.data_value))
						..code_components.right_square_bracket
						..code_components.space
				)
				strObj = (
			...
			elseif (v.data_type == 'string') then
				strLine = (
					...
					--	..v.data_value
						...
						..tostring(i)
						..code_components.left_square_bracket
						..tostring(string.len(v.data_value))
						..code_components.right_square_bracket
				)
				strObj = (
					strObj
			...
			if (strObj) then
				if (string.len(strObj) > 0) then
					outObj = (outObj..strObj..";")
				end
			end
		end
		if (outObj) then
		...
		print_debug('nil_value: c_file_generator/c_class_data/arg/class_data')
	end
	return ({ atr = outAtr, fnc = outFnc, obj = outObj })
end

local c_class = function(class_name, class_data) 
	local outStr = ''
	if (class_name and class_data) then
		local cd = c_class_data(class_name, class_data)
		print(cd)
		outStr = (
			c_components.tipe
				..code_components.space
		...
		if (class_name == nil) then
			print_debug('nil_value: c_file_generator/c_class/arg/class_name')
		end
		if (class_data == nil) then
			print_debug('nil_value: c_file_generator/c_class/arg/class_data')
		end
	end
	return outStr
end

local c_demo_class = demo_class

local c_demo_class_data = c_class(
	c_demo_class.class_name, 
	c_demo_class.class_data
)


print(c_demo_class_data)
print('go')
